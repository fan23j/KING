...
def technical_quality(model, video_list, device, **kwargs):
    ...
    video_results = []
    for video_info in tqdm(video_list, disable=get_rank() > 0):
        ...
        logger.info(f"Video: {video_path}, Score: {acc_score_video/len(images)}")
...
def eval_imaging_quality(json_dir, device, submodules_list, **kwargs):
